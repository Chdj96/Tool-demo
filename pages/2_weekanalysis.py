...
# Updated create_gradient_plot to prevent cut-off in display and image
...
def create_gradient_plot(data_left, data_right=None, title="", param_left="", param_right=None, left_unit="",
    ...
    ax.set_ylim(0, mean_value * 1.3)  # increased from 1.2 for better spacing

    ax.set_title(title)
    ax.set_xlabel("Time")
    ax.set_ylabel(f"Value ({left_unit})" if not right_unit else f"Value ({left_unit}, {right_unit})")

    # Updated legend placement: top right inside
    ax.legend(title="Parameters", loc="upper right", fontsize='small', fancybox=True, shadow=True, frameon=True)

    # Save with tight layout for image output
    buf = io.BytesIO()
    fig.tight_layout()
    fig.savefig(buf, format="png", dpi=300, bbox_inches='tight', pad_inches=0.4)
    buf.seek(0)
