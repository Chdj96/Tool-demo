...
# Function to round time to the nearest quarter, half-hour, or full hour
def round_time(dt, base=15):
    """Rounds a given datetime to the nearest `base` minutes (15, 30, or 60)."""
    new_minute = (dt.minute // base) * base
    return dt.replace(minute=new_minute, second=0)

# Function to create time-based plot with rounded timestamps
def create_gradient_plot(data_left, data_right=None, title="", param_left="", param_right=None, left_unit="",
                         right_unit=None, show_thresholds=False, thresholds=None, start_time=None, end_time=None, rounding_base=30):
    ...
    # Ensure exactly 12 time intervals
    num_segments = 12
    tick_indices = np.linspace(0, len(data_left) - 1, num_segments, dtype=int)

    # Generate time labels, rounded to the nearest `rounding_base` minutes
...
if uploaded_file:
    ...
    column_data_right, maxVal_right, AvgVal_right, minVal_right, number_of_points_right = None, None, None, None, None
    if right_param:
    ...
    create_gradient_plot(
        ...
        data_right=AvgVal_right if right_param else None,
